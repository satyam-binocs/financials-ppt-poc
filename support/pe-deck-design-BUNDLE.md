...
and the entire scale collapses. `pe_components.js` sets this before the first slide exists;
...
content spec (JSON)
   ...
   │
   ├─2─► pe_components.js    measure → distribute → render  (writes the .pptx)
   │
   ├─3─► audit_deck.py       compliance audit against the emitted XML
   │
   └─4─► render + look       soffice → pdftoppm → view every slide
```

```bash
python3 scripts/layout_engine.py spec.json               # 1. plan
node    scripts/example_build.js deck.pptx               # 2. build (copy this file)
python3 scripts/audit_deck.py deck.pptx                  # 3. audit
python3 /mnt/skills/public/pptx/scripts/office/validate.py deck.pptx   # file integrity
```

`layout_engine.py --selftest` verifies the engine against its own rules; run it if you
change a token.
...
literally horizontal/vertical font scaling, which the spec forbids; `audit_deck.py` fails
...
Read `scripts/example_build.js` before writing a new deck. The shape of those calls is the
...
`layout_engine.py` and `pe_components.js` collapse runs identically, so the two never disagree.
...
eyebrows (≤18 chars). **Never Title Case.** `audit_deck.py` flags both violations.
...
| `scripts/pe_components.js` | Stage 2. Two-phase layout + component library. |
| `scripts/audit_deck.py` | Stage 3. Post-build compliance audit. |
| `scripts/example_build.js` | Worked five-slide reference deck. Start here. |
...
class Slide {
  ...
  get width() { return this.g.content_width_in; }
  get left() { return this.g.margin_left_in; }
  ...
  _eyebrow(text) {
    const clean = String(text).slice(0, TOKENS.type.case.allcaps_max_chars).toUpperCase();
...
def check_case(root, sn, tokens, out):
...
Usage: python3 extract_bundle.py pe-deck-design-BUNDLE.md [output_dir]
"""
import re, sys, os

def main():
    bundle_path = sys.argv[1] if len(sys.argv) > 1 else "pe-deck-design-BUNDLE.md"
